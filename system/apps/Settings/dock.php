...
if (is_dir_empty($valuedir)) {
  echo 'Разделитель '.$dir_array[$keydir].'<br>';
}
else{
  echo 'Разделитель '.$dir_array[$keydir].'<br>';
  ...
  foreach (glob($valuedir.'*') as $key => $object) {
    ...
    echo'
  <div id="'.$ObjectName.$AppID.'" class="app-container'.$AppID.'" style="display:flex; padding:10px; border-bottom:1px solid #ccc; transition:all 0.1s ease-in;">
