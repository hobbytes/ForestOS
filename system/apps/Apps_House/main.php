...
<div id="'.$key['file'].'" class="app_h ui-forest-blink" '.$btnaction.' style="background-color:#'.$btncolor.'; color:#fff; font-size:13px; padding:5px;">'.$btntext.'</div></span>
...
<div id="updatetab<?echo $appid;?>">
  ...
  foreach ($arrayu as $key)
  {
    if($upd_array['subversion']!=$key['subversion']){
      ...
      <div id="'.$key['file'].'" class="app_h ui-forest-blink" onClick="update'.$appid.'()" style="background-color:#962439; color:#fff; width:30%; margin: 10px auto 10px auto; font-size:13px; padding:5px; text-align:center;">'.$apphouse_lang[$cl.'_upd_button'].'</div></span>
      ';
      }
    }
  ...
  foreach ($array as $key)
  {
    if (array_key_exists($key['file'], $ini_array))
    {
      ...
      if($newversion>$curversion){
        ...
        <span style="font-size:10px;">'.$apphouse_lang[$cl.'_card_version'].': '.$key['version'].'<br>'.$apphouse_lang[$cl.'_card_size'].': '.$format.'</span></div></span><br><div id="'.$key['file'].'" class="app_h ui-forest-blink" onClick="downloadapp(this,'.$newversion.');" style="background-color:#245896; color:#fff; font-size:13px; padding:5px;">'.$apphouse_lang[$cl.'_upd_button'].'</div></span>
