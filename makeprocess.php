...
$d=$_GET['d'];$i=$_GET['i'];$n=$_GET['n'];$p=$_GET['p'];$f=$_GET['f'];$k=$_GET['k'];
...
if(!is_file($destination.$file.'.php')){
  $destination=str_replace('system/apps//','',$destination); $destination=stristr($destination, 'system/');  $nameprocess=$file.$idprocess;
  if(!is_file($destination.$file.'.php')){
  $object->dialog("Файл <b>$destination$file.php</b> не существует!","Ошибка запуска программы","bounce");
exit(''); }}
if (is_file($destination.'app.png')){$appicon=$hashfile->filehash($destination.'app.png');}
else {
